...
| Two-model concurrent split | **Implemented, tested, and the harness-fault turn-budget bug it exposed is now fixed.** A real background thread runs the heavy model continuously in a producer/consumer pipeline: the interaction thread hands over the freshest observation every turn (`submit`), then waits only a short grace window (`WARM_GRACE_S = 0.3s`) for whatever the background thread has *already finished* from an earlier, strictly older submission (`take_ready_action`) -- an honest decision lag, never a fabricated instant answer. First measurement found every warm turn timing out and the episode failing outright on turn exhaustion (`real_collects = 0/29` on a 30-turn cap): a single background reasoning call takes 7-20s wall-clock, but the harness charged every `decide()` call -- including the harness's own synthetic "nothing's ready yet" filler tick -- as one unit against a hard-capped, discrete `max_turns` budget. That is a harness accounting bug, not a property of the architecture: in the target system, a non-blocking glance while the background model keeps thinking is not a decision, it costs wall-clock, which the harness already bills correctly and separately via `think_charge_s` (each call, filler or real, advances sim time by a realistic ~0.5s regardless of how long it actually took to compute). **Fix:** `backends_v3.py`'s `FILLER_ACTION` now carries a `_harness_filler` marker; `brain_agent.py`'s `_apply()` skips the `turns += 1` charge exactly when that marker is set, for this backend only -- every other backend's accounting is untouched, since for them one call really is one decision. Verified on `counter_within_reach` at its real 420s/46-turn budget: 968 total `decide()` calls, 933 free filler ticks, only 35 counted as real decisions -- well under the cap -- and the episode ran its *entire* sim-time budget and ended on `time limit`, exactly like every other backend's episodes do, instead of starving on an artifact of call-counting. Turn budget is no longer the constraint that binds; sim-time -- the real per-episode deadline -- is, same as it is for the robot in reality. That episode still did not complete the task (0/2 goals), but for an unrelated, separate reason surfaced only because the fix let the episode run long enough to reach it: the background reasoner's `ground_object` tool (VLM height/distance estimate, then `check_reach` against the real geometry in `reach_tool.py`) reported the jar as out of reach at an estimated 0.45m when it actually sits at 0.194m -- a wrong perception judgment, not a scheduling one, and `counter_within_reach` exists specifically to catch exactly that mistake (T-13/pair-challenge design, see the challenge file's own docstring). Filed as its own open item in FINDINGS.md, not conflated with the turn-budget fix, which is confirmed working. |
...
Backend: `sim/bench/backends_v2.py`, class `NemotronStackBackend`, in its
own `BACKENDS_V2` registry, merged with the existing Codex/Gemini/Echo
registry inside `main.py` (deliberately an addition, not an edit to
`backends.py` -- see the merge-site comment there) -- runnable as
`--agents brain:nemotron_stack` through the unmodified harness.
