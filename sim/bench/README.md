...
`--map apartment` adds the 77 legacy, uncategorised probes that predate the
bundles; they load fine but report in the uncategorised bucket, so they are
opt-in rather than silently inflating "everything" to 122 episodes.
...
`run_bench.sh`, `innate_up.sh`, `prime_brain.sh`, `say_brief.sh`) exactly as
...
- **`eff` is trivially 1.00.** RHAE's `h` is meant to be the second-best
  *human's* action count. No human data has been collected, so the oracle's own
  step count stands in and it is scoring itself. It only becomes meaningful
  once a real agent runs.
