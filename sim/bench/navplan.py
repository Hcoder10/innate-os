...
class NavMap:
    def __init__(self, grid: np.ndarray, origin_x: float, origin_y: float, resolution: float):
        ...
        # Keep the raw mask too: goals often sit ON furniture (a mug on a
        # bench), so the goal cell itself may be inflated-blocked while the
        # approach beside it is fine.
        self.raw_blocked = blocked
