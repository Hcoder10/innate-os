...
async def activate_brain(rosbridge: str) -> str:
    """Turn the brain on. It boots INACTIVE and silently drops every instruction
    with "Brain is not active" until something flips it -- the web UI has a
    toggle, a headless harness has to call the service itself."""
    import websockets

    try:
        async with websockets.connect(rosbridge, max_size=None, ping_interval=None, open_timeout=10) as ws:
            await ws.send(json.dumps({
                "op": "call_service", "service": "/brain/set_brain_active",
                "args": {"data": True}, "id": "activate",
            }))
            reply = json.loads(await asyncio.wait_for(ws.recv(), timeout=25))
        return "" if reply.get("result", True) else str(reply)
    except Exception as exc:  # noqa: BLE001
        return f"{type(exc).__name__}: {exc}"


async def reset_brain(rosbridge: str) -> str:
    """Clear the agent's conversation before the next challenge.

    The world resets on challenge start; the BRAIN does not. Without this the
    agent carries the previous task's history into the next one and can answer
    from it, which is contamination the scores would silently absorb.
    """
    import websockets
    ...
    try:
        async with websockets.connect(rosbridge, max_size=None, ping_interval=None, open_timeout=10) as ws:
            await ws.send(json.dumps({
                "op": "call_service", "service": "/brain/reset_brain", "args": {}, "id": "reset",
            }))
...
def _blocked_here(ids: list[str]) -> dict[str, str]:
    ...
    32 of those 77 need a pick, and at the default 900s timeout that is hours
    of timeouts producing zeros, which is the exact outcome this exists to
    prevent.
    """
    try:
