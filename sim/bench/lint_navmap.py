...
that extend past the walls make it fatal.
  ...
  usage: lint_navmap.py <map.yaml> [<x> <y>]     # x,y: a pose to route from
"""
...
def apron_area(free: np.ndarray, occupied: np.ndarray, res: float) -> float:
...
def bfs(passable: np.ndarray, start: tuple[int, int]) -> np.ndarray:
    """8-connected geodesic distance in CELLS (diagonals cost sqrt(2))."""
    dist = np.full(passable.shape, np.inf, dtype=np.float32)
    if not passable[start]:
        return dist
    dist[start] = 0.0
    # Dial-style: a plain deque is enough at this size and needs no heapq churn.
    queue = deque([start])
    ...
    while queue:
        ...
        for dr, dc, cost in steps:
            nr, nc = r + dr, c + dc
            if 0 <= nr < h and 0 <= nc < w and passable[nr, nc] and base + cost < dist[nr, nc] - 1e-6:
                dist[nr, nc] = base + cost
                queue.append((nr, nc))
    return dist
...
def main() -> int:
    ...
    apron_m2 = apron_area(read_free, lethal, res)
