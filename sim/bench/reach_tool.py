...
says "reachable" is never wrong about what the judge will accept.
...
from brain_agent import ARM_Z_MAX_M, PICK_REACH_M, PLACE_AHEAD_M
...
def can_place(robot_xy: tuple[float, float], dest_xy: tuple[float, float], heading_rad: float,
              dest_min_z: float | None = None) -> Verdict:
    """Verdict for whether a `place` FROM HERE, facing `heading_rad`, drops
    the carried object at dest_xy -- i.e. does the fixed PLACE_AHEAD_M offset
    land where the goal wants it, not just somewhere pickable-adjacent.
    dest_min_z is None when the destination has no height requirement
    (a floor drop) -- see the InCircle.min_z contract (CHANGES.md,
    patch_goal_height), which this mirrors so a caller never disagrees with
    the judge."""
    px = robot_xy[0] + PLACE_AHEAD_M * math.cos(heading_rad)
    py = robot_xy[1] + PLACE_AHEAD_M * math.sin(heading_rad)
    d = math.hypot(px - dest_xy[0], py - dest_xy[1])
    ok = d <= 0.15  # a place goal's own radius is the judge's business; this
    # tool only answers "does the fixed 30 cm offset land near the aim point"
    return Verdict(ok, round(d, 3), dest_min_z,
                    "aim point lands within 15 cm of target" if ok else
                    f"aim point is {d:.2f} m off target -- adjust heading or approach distance")
