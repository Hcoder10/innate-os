...
`lint_reach.py` needs exactly the same "must this object move?" judgement, and
imports `needs_move` from here rather than keeping its own copy -- two
implementations of a rule this fiddly would eventually disagree, and the one
that disagreed silently would be the one shipping numbers.
"""
...
def needs_move(challenge, prop: str | None = None) -> bool:
    ...
    With `prop`, asks about that object alone -- which is what lint_reach wants,
...
def needs_manipulation(challenge) -> bool:
    ...
    the skill reporting success and by nothing else. `laundry_claim_only`'s
    only goal is exactly that, and geometry alone reads it as needing no pick.

    The name check lives here and NOT in `needs_move`, because `needs_move`
    also answers lint_reach's per-prop question -- "must THIS object be within
    reach?" -- and a skill name is not a prop. Keeping it out preserves that
    function's behaviour exactly, which was verified equal to lint_reach's
...
def _ungraspable() -> dict[str, str]:
    """Challenges whose target does not fit the gripper, from lint_grasp.
    ...
    episode. Regenerate with `lint_grasp.py --manifest` after touching props.
    """
