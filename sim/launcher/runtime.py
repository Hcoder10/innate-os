    ...
    if container_was_running:
        ...
        if offline:
            ...
            try:
                ...
                os_image = resolve_local_os_image(os_repo)
                up_cmd.append("--build")
            else:
                up_cmd.append("--no-build")
...
def collect_os_process_status(config: dict[str, object]) -> dict[str, bool]:
    ...
    output = capture_command_output(
        os_compose_zsh_cmd(
            ...
            "pgrep -f '[v]irtual_mars_node.py' >/dev/null; echo simdriver=$?"
        ),
        cwd=os_repo,
        env=compose_env,
    )
...
VIRTUAL_MARS_PROBE_CMD = "timeout 8 ros2 topic echo /odom --once >/dev/null 2>&1 && echo VIRTUAL_MARS_OK"
