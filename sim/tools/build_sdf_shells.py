...
Output: sim/assets/sdf_shells/<room>.obj (+ manifest-ready file sizes printed).
...
OUT_DIR = Path(__file__).resolve().parent.parent / "assets" / "sdf_shells"
...
def build_shell(room_obj: Path) -> Path:
    ...
    if len(shell.faces) > TARGET_FACES:
        ...
        m = ms.current_mesh()
        shell = trimesh.Trimesh(vertices=m.vertex_matrix(), faces=m.face_matrix(), process=False)
        print(
