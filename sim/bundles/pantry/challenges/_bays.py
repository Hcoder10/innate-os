...
numbers were checked with sim/bench/lint_reach.py, not by eye.
"""
...
# Where a thing counts as "put away" in each bay. Generous radii: the place
# skill releases at a fixed offset from the base, so demanding better measures
# the arm's calibration rather than whether the robot understood which bay.
PUT_A = (BAY_A_X, 0.0, 0.75)   # x, y, radius -- anywhere along the jar bay
PUT_B = (0.0, BAY_B_Y, 0.85)

# The bays as RECTS covering the whole rack shelf, with a height floor so the
# ground never counts. The circles above cover barely a third of each rack:
# a probe agent shelved the delivery carton "immediately beside the tea
# carton" -- the literal brief, on the carton shelf -- and scored 0 because
# the tea carton sits at x=0.75 and the circle ends at 0.85. "With the other
# cartons" means the carton bay, so the goal is the bay. (x0, y0, x1, y1).
