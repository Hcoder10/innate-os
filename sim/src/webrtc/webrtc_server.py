...
class WebRTCManager:
    ...
    async def _build_peer(self, client_id: str, active: list[str]) -> _Peer:
        # Use aiortc's default ICE config so the server gathers a server-reflexive
        # (srflx) candidate via public STUN. Remote browsers behind NAT need the
        # server's public IP, and host candidates alone don't advertise it — dropping
        # STUN breaks remote WebRTC for anyone not on an address-restricted NAT.
        # (Public STUN's unreachable retries can log an aioice teardown warning —
        # Transaction.__retry; that's cosmetic and handled separately, not by removing
        # STUN.) The local _StunResponder still serves the browser's own reflexive lookup.
...
async def _main(shared_queues) -> None:
    ...
    while not shared_queues.exit_event.is_set():
        ...
        # Drain whatever else queued up (a START's peer build blocks this loop
        # for seconds — non-trickle ICE gathering — so messages pile up behind
        # it) and drop signaling superseded by a newer START from the same
        # client. Without this, a client that re-STARTs while a build is in
        # flight stays one offer behind: its answer to the stale offer gets
        # applied to the FRESH peer (mismatched ICE credentials), poisoning it
        # into a permanent "checking" state.
        items = [item]
        while True:
            try:
                items.append(sig_in.get_nowait())
            except queue.Empty:
                break

        for pending in _coalesce(items):
            try:
                await _dispatch(manager, pending, sig_out)
            except Exception as exc:  # one bad message must not kill the server
                print(f"[WebRTC] error handling {pending.get('kind')!r}: {exc}")
...
def _coalesce(items: list[dict]) -> list[dict]:
    ...
    kept = []
    for index, item in enumerate(items):
        newest = latest_start.get(item.get("client_id", ""))
        if newest is not None and index < newest:
            print(f"[WebRTC] dropping superseded {item.get('kind')!r} (newer START queued)")
            continue
        kept.append(item)
    return kept
...
async def _dispatch(manager: WebRTCManager, item: dict, sig_out: queue.Queue) -> None:
    ...
    if kind == "start":
        ...
        if offer_sdp is not None:  # None = no-reneg active-set change, no offer to send
            # Build time is dominated by ICE gathering (aiortc can't trickle);
            # the webapp's offer watchdog must stay above the worst case seen here.
            print(f"[WebRTC] peer for {client_id[:8]} built in {time.monotonic() - started_at:.1f}s")
