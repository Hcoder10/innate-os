...
- **observer stream** (WebSocket, port 8800; the webapp proxies it at
  ...
  every physics slice (~75Hz), latest-wins per client, with the connection's
  first frame also carrying `object_specs` (the drawable prop roster viewers
  build their meshes from), and stage commands
