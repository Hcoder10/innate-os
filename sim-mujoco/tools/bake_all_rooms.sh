...
# in decompose_rooms.py. Logs: work/decomp400_<room>.log. Interrupting is
...
for room in $ROOMS; do
  uv run decompose_rooms.py "$room" > "../work/decomp400_$room.log" 2>&1 &
done

trap 'kill 0' INT TERM  # Ctrl-C kills all room workers, not just the wait

echo "baking $(echo "$ROOMS" | wc -w | tr -d ' ') rooms in parallel; tail -f work/decomp400_<room>.log for progress"
wait
echo "=== all rooms done ==="
for room in $ROOMS; do
  printf '%s: %s\n' "$room" "$(tail -1 "../work/decomp400_$room.log")"
done
