...
sim-web renders (via Three.js's GLTFLoader) into OBJ+JPEG files MuJoCo can
...
Usage: uv run export_visual_rooms.py [out_dir]
    (default out_dir: work/apartment_visual)
"""
...
GLB_PATH = (
    Path(__file__).resolve().parent.parent.parent / "sim-web" / "public" / "models" / "appartement.glb"
)
...
def main() -> None:
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent / "work" / "apartment_visual"
