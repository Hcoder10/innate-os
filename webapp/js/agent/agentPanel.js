...
export function createAgentPanel(root, rosClient, agentState) {
  ...
  /** Display name for a directive id, falling back to the id. @param {string} id */
  function directiveName(id) {
    const a = agentState.get().agents.find((x) => x.id === id);
  ...
  toggleBtn.addEventListener("click", () => {
    const { brainActive, currentDirective } = agentState.get();
    if (brainActive) {
      // Echo the user's stop in the chat (named), then deactivate the brain.
      addMessage("system", `${directiveName(currentDirective || lastDirective)} stopped.`, Date.now() / 1000);
      void withApplying(() => agentState.setDirective(""));
    } else {
      const id = directiveSelect.value || lastDirective;
      if (id) {
        addMessage("system", `${directiveName(id)} started.`, Date.now() / 1000);
        void withApplying(() => agentState.setDirective(id));
      }
    }
  });
