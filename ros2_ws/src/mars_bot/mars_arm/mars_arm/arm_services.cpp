...
void MarsArmNode::configureServosLocked(bool enable_torque) {
...
void MarsArmNode::syncTargetToMotorPositions() {
...
std::string MarsArmNode::describeHardwareError(uint8_t status, int servo_id) const {
...
void MarsArmNode::armCommandCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
    try {
        if (msg->data.size() != 6) {
