    ...
    for root in roots:
        ...
        for path in sorted(root.glob("*.py")):
            if path.name.startswith("_"):
                continue
            try:
                spec = importlib.util.spec_from_file_location(f"sim_room_{path.stem}", path)
                assert spec and spec.loader
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
                room: Room = module.ROOM
                room.root = path.parent
                found[room.name] = room
            except Exception as exc:  # noqa: BLE001 -- a bad sidecar loses its room, nothing else
                print(f"[rooms] skipping {path.name}: {exc!r}", flush=True)
    return found
