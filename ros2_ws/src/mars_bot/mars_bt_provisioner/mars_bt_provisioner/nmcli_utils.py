...
def _run_nmcli(command_list, timeout=10, check=True, capture_output=True, use_sudo=False):
    ...
    try:
        result = subprocess.run(command_list, capture_output=capture_output, text=True, check=check, timeout=timeout)
    ...
    for name, uuid in wifi_connections_to_query:
        ...
        if success_detail and stdout_detail:
            try:
                ...
                nm_logger.warning(
                    f"Could not parse priority for connection {name} (UUID: {uuid}): {stdout_detail}. Error: {e}. Using default priority 0."
                )
                priority = 0  # Fallback to default
        else:
            nm_logger.warning(
                f"Failed to get priority for connection {name} (UUID: {uuid}). Error: {stderr_detail or 'Unknown error'}. Using default priority 0."
            )
            priority = 0  # Fallback to default
...
def nmcli_add_or_modify_connection(ssid, password, priority, autoconnect=True):
    ...
    if exists:
        ...
        success_del, _, stderr_del = _run_nmcli(["nmcli", "connection", "delete", ssid], use_sudo=True)
        if not success_del:
...
def nmcli_connect(ssid, ifname=DEFAULT_WIFI_INTERFACE):
    ...
    nm_logger.info(
        f"Attempting to connect to network profile: {ssid}{f' on interface {target_interface}' if target_interface else ''}"
    )
...
def nmcli_get_active_ipv4_address():
    ...
    success_ip, stdout_ip, stderr_ip = _run_nmcli(["nmcli", "-t", "-f", "IP4.ADDRESS", "device", "show", active_device])
    if not success_ip:
