...
Signalling has two paths (matching the node): no client_id -> bare topics (legacy single peer);
client_id present -> ``*_id`` topics with ``{client_id, ...}`` envelopes (independent concurrent peers).
...
OFFER, OFFER_ID = "/webrtc/offer", "/webrtc/offer_id"
...
class Driver:
    ...
    def __init__(self):
        ...
        self.status = {}
        self.offers = {}        # client_id (or "") -> latest offer SDP
        self._lock = threading.Lock()
        self.node.create_subscription(String, ACTIVE, self._on_status, 10)
        self.node.create_subscription(String, OFFER, self._on_offer_bare, 10)
    ...
    def start(self, client_id=None, source="live", video=None, audio=False):
        p = {"source": source, "audio": audio}
    ...
    def start_get_offer(self, client_id, video, tries=4, per_try=2.0):
        ...
        for _ in range(tries):
            ...
            if wait_for(lambda: self.offers.get(key) not in (None, ""), per_try):
...
def test_legacy(d):
    print("\n[legacy] no client_id -> bare raw-SDP offer (backward compat)")
    raw = d.start_get_offer(None, ["arm"]) or ""
    is_raw_sdp = raw.startswith("v=0")  # raw SDP, not a JSON envelope
    print(f"  bare /webrtc/offer is raw SDP: {is_raw_sdp}  (m-lines={offer_mlines(raw)})")
    d.release(None)
    print(f"  {'PASS' if is_raw_sdp else 'FAIL'}")
    return is_raw_sdp
...
def main():
    ...
    ap.add_argument("mode", nargs="?", default="full",
                    choices=["full", "fanout", "gating", "selective", "timeout", "legacy"])
    ...
    try:
        ...
        fns = {"fanout": lambda: test_fanout(d, args.peers), "gating": lambda: test_gating(d),
               ...
               "legacy": lambda: test_legacy(d), "full": lambda: run_full(d, args)}
