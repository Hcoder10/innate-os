...
**`*_id`** topics. Clients that do not send a `client_id` will not connect on this
branch.
...
|---|---|---|
| START client→node | `/webrtc/start` | `{client_id, source, video, audio, ...}` |
| Offer node→client | `/webrtc/offer_id` | `{client_id, sdp}` |
| Answer client→node | `/webrtc/answer_id` | `{client_id, sdp}` |
| ICE node→client | `/webrtc/ice_out_id` | `{client_id, candidate, sdpMLineIndex}` |
| ICE client→node | `/webrtc/ice_in_id` | `{client_id, candidate, sdpMLineIndex, sdpMid}` |
