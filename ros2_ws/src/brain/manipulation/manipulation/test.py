...
class ImageCapture(Node):
    def __init__(self):
        ...
        image_qos = QoSProfile(reliability=ReliabilityPolicy.BEST_EFFORT, history=HistoryPolicy.KEEP_LAST, depth=2)
    ...
    def check_and_process_images(self):
        ...
        if (
            self.first_live_image1 is not None
            and self.first_live_image2 is not None
            and not self.images_captured
            and self.hdf5_image1 is not None
            and self.hdf5_image2 is not None
        ):
    ...
    def process_and_display_all_images(self):
        ...
        try:
            ...
            print("=" * 50)
            print(
                f"HDF5 Camera 1 - Shape: {hdf5_img1_resized.shape}, Range: {hdf5_img1_resized.min()}-{hdf5_img1_resized.max()}"
            )
            print(
                f"HDF5 Camera 2 - Shape: {hdf5_img2_resized.shape}, Range: {hdf5_img2_resized.min()}-{hdf5_img2_resized.max()}"
            )
