        ...
        for key, ft in self.features.items():
            ...
            if norm_mode == NormalizationMode.MEAN_STD:
                ...
                if torch.isinf(mean).any():
                    raise ValueError(_no_stats_error_str(f"{key} mean"))
                ...
                if torch.isinf(max_val).any():
                    raise ValueError(_no_stats_error_str(f"{key} max"))
    ...
    def _get_policy_features(self, shapes_dict: dict[str, list[int]]) -> dict[str, PolicyFeature]:
        ...
        for key, shape in shapes_dict.items():
            ...
            if ftype:
                ...
                print(
                    f"Warning: Could not infer FeatureType for '{key}'. It will not be normalized/unnormalized by default."
                )
        return features
...
class ACTPolicy(nn.Module):
        ...
        if self.config.image_input_keys:
            ...
            for key in self.config.image_input_keys:
                if key in processed_batch:
                    ...
                    print(
                        f"Warning: Image key '{key}' declared in 'image_input_keys' but not found in batch for _prepare_batch_for_model."
                    )
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if "action_is_pad" in working_batch:  # Use padding info from the (potentially modified) working_batch
            ...
            l1_loss = (l1_loss * ~action_is_pad.unsqueeze(-1)).sum() / (
                (~action_is_pad.unsqueeze(-1)).sum() + 1e-8
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.use_vae:
            ...
            self.vae_encoder_action_input_proj = nn.Linear(self.config.output_shapes["action"][0], config.dim_model)
        ...
        if self.config.image_input_keys:
            ...
            try:
                ...
                if weights_arg == "ResNet18_Weights.IMAGENET1K_V1":  # example
                    ...
                    print(
                        f"Warning: Pretrained backbone weights '{weights_arg}' might not be directly loadable as a string. Consider using torchvision.models.weights enums."
                    )

            except ImportError:
                if weights_arg is not None:
                    print(
                        f"Warning: torchvision.models.weights enums not found (likely older torchvision). Pretrained backbone weights '{weights_arg}' passed as string."
                    )
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor | None, Tensor | None]]:
        ...
        pos_embed_1d_components = [
            self.encoder_1d_feature_pos_embed.weight[current_1d_token_idx]
            .unsqueeze(0)
            .unsqueeze(0)
        ...
        if "observation.state" in batch and hasattr(self, "encoder_robot_state_input_proj"):
            ...
            pos_embed_1d_components.append(
                self.encoder_1d_feature_pos_embed.weight[current_1d_token_idx]
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(1, batch_size, 1)
        ...
        if (
            self.config.image_input_keys
            and "observation.images" in batch
