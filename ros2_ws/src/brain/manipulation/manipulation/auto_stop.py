...
class LearnedStopDetector:
    ...
    def _smooth(self, progress: float | None) -> float | None:
        """EMA-smooth progress; carry the last value forward when progress is None."""
        if progress is None:
    ...
    def update(self, signals: StepSignals, elapsed: float, now: float):
        ...
        # (first step, or no progress head) count as "moving" so we never stop on
        # them. The timer runs through the min_duration window; the floor only gates
        # the decision below, not the accumulation.
        moving = (
