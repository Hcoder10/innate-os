    ...
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info(
                f"Loaded {len(self.episodes)} episodes from {self.metadata.get('task_name', 'Unknown Task')}"
            )
    ...
    def play_episode(self, file_path, episode_num, total_episodes, episode_id):
        ...
        try:
            for t in range(num_timesteps):
                ...
                cv2.putText(
                    disp_img, f"qpos: {qpos[t]}", (x, y0), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), thickness
                )
                cv2.putText(
                    disp_img,
                    f"cmd_vel: {qvel[t]}",
                ...
                cv2.putText(
                    disp_img,
                    "Window Active - Press 'q' to quit",
                    (10, disp_img.shape[0] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (0, 255, 0),
                    1,
                ...
                if t % progress_interval == 0 or t == num_timesteps - 1:
                    ...
                    self.get_logger().info(
                        f"Episode {episode_num}/{total_episodes} (ID: {episode_id}) - Frame {t + 1}/{num_timesteps} ({progress:.1f}%) [Speed: {self.playback_speed}x]"
                    )
