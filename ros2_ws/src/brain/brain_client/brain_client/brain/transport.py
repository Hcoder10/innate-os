...
# unaffected.
_USAGE_LOG = _os.environ.get("GEMINI_USAGE_LOG", "/root/innate-os/workspace/gemini_usage.jsonl")
...
def direct_rest(api_key: str) -> GeminiRest:
    ...
    def request(method: str, path: str, body: dict | None = None) -> dict:
        resp = _rest_send(lambda: client.request(method, DIRECT_BASE_URL + path, json=body))
        if resp.status_code != 200:
