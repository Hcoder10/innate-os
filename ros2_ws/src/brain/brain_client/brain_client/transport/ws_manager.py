...
class WebSocketManager:
    ...
    def _send(self, message_in) -> None:
        if not self.ws_client:
            ...
            # Normal before the first connect: the registration payload is
            # re-sent once the socket opens (orchestrator.on_ws_status).
            self.get_logger().info("WebSocket not connected yet; dropping message")
        else:
