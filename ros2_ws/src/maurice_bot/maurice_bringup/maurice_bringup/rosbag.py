...
def extract_chat_dialogue(bag_path, output_file):
    ...
    converter_opts = rosbag2_py.ConverterOptions(input_serialization_format="cdr", output_serialization_format="cdr")
    ...
    chat_topics = {topic: msg_type for topic, msg_type in topics_and_types.items() if "chat" in topic.lower()}
    ...
    while reader.has_next():
        ...
        if topic in chat_topics:
            ...
            chat_messages.append({"timestamp": timestamp, "speaker": speaker, "content": content, "topic": topic})
...
def extract_video(bag_path, output_file, fps=30):
    ...
    if not image_topics:
        print(
            "No mars/main_camera/left/image_raw/compressed or mars/arm/image_raw/compressed topics found in the bag file."
        )
        print("Available topics:", list(topics_and_types.keys()))
        return
...
def select_and_process_bag():
    ...
    bag_path = filedialog.askdirectory(title="Select ROS bag directory", initialdir=os.getcwd())
...
def main():
    ...
    parser.add_argument(
        "--manual", action="store_true", help="Use manual mode with command line arguments instead of dialog"
    )
    parser.add_argument("bag_path", nargs="?", help="Path to the ROS bag directory (manual mode only)")
    parser.add_argument(
        "-o",
        "--output",
        default="chat_dialogue.txt",
        help="Output text file (manual mode only, default: chat_dialogue.txt)",
    )
    parser.add_argument("-v", "--video", help="Output MP4 video file (manual mode only)")
    parser.add_argument("--fps", type=int, default=30, help="Frames per second for video output (default: 30)")
    parser.add_argument("--both", action="store_true", help="Extract both chat and video (manual mode only)")
