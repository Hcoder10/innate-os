...
def generate_launch_description():
    ...
    width_arg = DeclareLaunchArgument(
        "width", default_value="1280", description="Camera capture width (640x480 per side)"
    )

    height_arg = DeclareLaunchArgument(
        "height", default_value="480", description="Camera capture height (640x480 per side)"
    )

    publish_left_width_arg = DeclareLaunchArgument(
        "publish_left_width", default_value="640", description="Publish left image width"
    )

    publish_left_height_arg = DeclareLaunchArgument(
        "publish_left_height", default_value="480", description="Publish left image height"
    )

    publish_stereo_width_arg = DeclareLaunchArgument(
        "publish_stereo_width", default_value="1280", description="Publish stereo image width"
    )

    publish_stereo_height_arg = DeclareLaunchArgument(
        "publish_stereo_height", default_value="480", description="Publish stereo image height"
    )

    fps_arg = DeclareLaunchArgument("fps", default_value="30.0", description="Camera frame rate")

    frame_id_arg = DeclareLaunchArgument(
        "frame_id", default_value="camera_optical_frame", description="Camera frame ID"
    )

    jpeg_quality_arg = DeclareLaunchArgument(
        "jpeg_quality", default_value="80", description="JPEG compression quality (1-100)"
    )

    publish_compressed_arg = DeclareLaunchArgument(
        "publish_compressed", default_value="true", description="Publish compressed image topic"
    )

    compressed_frame_interval_arg = DeclareLaunchArgument(
        "compressed_frame_interval", default_value="6", description="Publish compressed image every N frames"
    )

    # V4L2 control parameters
    exposure_arg = DeclareLaunchArgument(
        "exposure", default_value="-1", description="Manual exposure time (-1 = use current value, 1-10000)"
    )

    gain_arg = DeclareLaunchArgument(
        "gain", default_value="-1", description="Manual gain value (-1 = use current value, 0-255)"
    )

    default_gain_arg = DeclareLaunchArgument(
        "default_gain", default_value="110", description="Default gain value for auto-exposure mode (0-255)"
    )
