...
def generate_launch_description():
    ...
    maurice_arm_share = get_package_share_directory("maurice_arm")  # noqa: F841
    maurice_sim_share = get_package_share_directory("maurice_sim")

    # Get the path to the arm config file (ROS 2 param format)
    arm_config_file = PathJoinSubstitution([FindPackageShare("maurice_arm"), "config", "arm_config.yaml"])
    ...
    return LaunchDescription([maurice_arm_node])
