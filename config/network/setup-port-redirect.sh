#!/bin/sh
# Redirect privileged ports 443 -> 4443 and 80 -> 4080, the unprivileged ports
# the webapp proxy binds (proxy/https_server.py runs as the jetson1 user and
# can't bind a port below 1024). With this in place the cockpit is reachable at
# https://<robot>.local (and http://<robot>.local) with no port in the URL,
# while the proxy stays unprivileged.
#
...
# Each entry is "<privileged-port> <proxy-port>".
for MAP in "443 4443" "80 4080"; do
	set -- $MAP
	RULE="PREROUTING -p tcp --dport $1 -j REDIRECT --to-ports $2"
