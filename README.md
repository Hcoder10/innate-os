...
If you don't have a robot, this repository now includes the simulator as [`sim/`](sim/) and a repo-local `./innate sim` workflow for setup and startup.
...
- [`.env.template`](.env.template) copied to `.env` for secrets only, typically `INNATE_SERVICE_KEY`
- [`config/os.toml.template`](config/os.toml.template) copied to `config/os.toml` for optional non-secret OS overrides such as the brain websocket, telemetry URL, Cartesia voice, and Uninavid URL
...
- Keep secrets in `.env` and optional non-secret runtime overrides in `config/os.toml`.
