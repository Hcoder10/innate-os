// Dynamixel Protocol 2.0 over WebSerial -- reads the USB leader arm's six
// servos. Ported from webapp/js/dynamixel.js (the operator webapp's leader-
// arm teleop), which see for the protocol writeup; kept byte-for-byte
// identical here so this sandbox tracks the real robot's leader-arm input.
//
...
// them through untouched, exactly like webapp's armPanel.js does.
//
...
export function buildPacket(id: number, instruction: number, params: number[]): Uint8Array {
...
export function buildSyncRead(ids: number[], addr: number, len: number): Uint8Array {
...
export interface StatusPacket {
  id: number;
  error: number; // Hardware/processing error byte (0 = ok).
  params: Uint8Array;
}
 ...
 */
export function createStatusParser(onPacket: (packet: StatusPacket) => void): (chunk: Uint8Array) => void {
...
export class DynamixelLeader {
  ...
  #listeners = new Set<(state: LeaderArmState) => void>();
  #state: LeaderArmState = { connected: false, positions: null, rate: 0, error: null };
  #ids: number[];
  #running = false;
  #opening = false;
  #round = new Map<number, number>();
  #roundComplete: (() => void) | null = null;
  #emitTimes: number[] = [];

  constructor(ids: number[] = LEADER_SERVO_IDS) {
    this.#ids = ids;
  }

  get state(): LeaderArmState {
    return this.#state;
  }

  /** @param cb Fires immediately, then on change. @returns unsubscribe */
  onChange(cb: (state: LeaderArmState) => void): () => void {
  ...
  async open(port: SerialPort): Promise<void> {
    // Coalesce concurrent opens -- see webapp/js/dynamixel.js's `open` for
    // the TOCTOU rationale (button + serial 'connect' event + getPorts()
    // on mount can all fire around the same cold-load moment).
  ...
  #patch(patch: Partial<LeaderArmState>): void {
